...
- (void)api_loadDetail
{
    ...
    [YXGeomorphySvyLineModel requestDetailWithUUID:self.forumListModel.uuid completion:^(YXGeomorphySvyLineModel * _Nonnull m, NSError * _Nonnull error) {
        if (error) {
...
- (void)setUpUIByModel:(id)model
{
    ...
    //微地貌测量工程编号
    [self.textFields textFieldForTag:3].text = m.geomorphysvyprjid;
    //野外编号
    [self.textFields textFieldForTag:4].text = m.fieldid;
    //测线名称
    [self.textFields textFieldForTag:5].text = m.name;
    //典型照片原始文件编号
    [self.textFields textFieldForTag:6].text = m.photoArwid;
    //照片镜像
    [self.labels labelForTag:0].text = m.photoviewingtoName;
    ...
    //拍摄者
    [self.textFields textFieldForTag:7].text = m.photographer;
}

//填充数据
- (id)buildBody
{
    YXGeomorphySvyLineModel *body = nil;
    if (self.interfaceStatus == InterfaceStatus_Edit) {
        ...
        body = [YXGeomorphySvyLineModel new];
    }
    //省
...
- (IBAction)onJingTouMirror:(UIButton *)b
{
    ...
    [YXSlectionDictModel requestDictWithType:FormType_GeomorphySvyLine code:@"CVD-16-Direction" completion:^(NSArray <YXSlectionDictModel *> * _Nonnull ms, NSError * _Nonnull error) {
        if (error) {
            [BDToastView showText:error.localizedDescription];
        }else{
            [BDToastView dismiss];
    ...
    [self judgeBaseDataWhenFinished:^(BOOL pass) {
        if (pass) {
            ...
            if (self.interfaceStatus == InterfaceStatus_Edit) {
                ...
                t.tableName = NSStringFromClass(YXGeomorphySvyLineModel.class);
                t.encodedData = [[self buildBody] yy_modelToJSONString];
        ...
        if (pass) {
            ...
            [self alertText:@"数据提交成功后将不能修改，您是否确认提交？" sureTitle:@"提交" sureAction:^{
                ...
                void (^submitRequest)(NSString *imgUrls) = ^(NSString *imgUrls) {
                    YXGeomorphySvyLineModel *body = [self buildBody];
