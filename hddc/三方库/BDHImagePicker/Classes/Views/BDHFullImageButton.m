...
- (UIButton *)fullImageButton {
    if (!_fullImageButton) {
        ...
        [_fullImageButton setTitle:NSLocalizedStringFromTable(@"fullImage", @"BDHImagePicker", @"原图") forState:UIControlStateNormal];
        _fullImageButton.titleLabel.font = kBDHFullImageButtonFont;
...
- (CGFloat)fullImageButtonWidth {
    NSString *string = NSLocalizedStringFromTable(@"fullImage", @"BDHImagePicker", @"原图");
    CGRect rect = [string boundingRectWithSize:CGSizeMake(MAXFLOAT, 20) options:NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:kBDHFullImageButtonFont} context:nil];
