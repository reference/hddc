...
@interface GPGeologicalSvyPlanningPt ()

@end

@implementation GPGeologicalSvyPlanningPt
...
- (void)api_loadDetail
{
    ...
    [YXGeologicalSvyPlanningPtModel requestDetailWithUUID:self.forumListModel.uuid completion:^(YXGeologicalSvyPlanningPtModel * _Nonnull m, NSError * _Nonnull error) {
        if (error) {
...
- (id)buildBody
{
    YXGeologicalSvyPlanningPtModel *body = nil;
    if (self.interfaceStatus == InterfaceStatus_Edit) {
        ...
        body = [YXGeologicalSvyPlanningPtModel new];
    }
    //省
    ...
    [self judgeBaseDataWhenFinished:^(BOOL pass) {
        if (pass) {
            ...
            if (self.interfaceStatus == InterfaceStatus_Edit) {
                ...
                t.tableName = NSStringFromClass(YXGeologicalSvyPlanningPtModel.class);
                t.encodedData = [[self buildBody] yy_modelToJSONString];
        ...
        if (pass) {
            [self alertText:@"数据提交成功后将不能修改，您是否确认提交？" sureTitle:@"提交" sureAction:^{
                ...
                void (^submitRequest)(NSString *imgUrls) = ^(NSString *imgUrls) {
                    YXGeologicalSvyPlanningPtModel *body = [self buildBody];
