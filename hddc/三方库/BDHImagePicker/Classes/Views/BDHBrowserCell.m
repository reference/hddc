...
#import "BDHTapDetectingImageView.h"
#import "BDHPhotoBrowser.h"
#import "UIView+BDHImagePicker.h"
#import "BDHAsset.h"
#import "BDHImagePickerHelper.h"

@interface BDHBrowserCell () <UIScrollViewDelegate,BDHTapDetectingImageViewDelegate>
@property (nonatomic, strong) UIScrollView *zoomingScrollView;
@property (nonatomic, strong) BDHTapDetectingImageView *photoImageView;

@end

@implementation BDHBrowserCell
...
- (void)displayImage {
    ...
    [BDHImagePickerHelper fetchImageWithAsset:self.asset targetSize:self.zoomingScrollView.size needHighQuality:YES imageResutHandler:^(UIImage * _Nonnull image) {
        if (!image) {
            return;
        }
...
#pragma mark - get
- (BDHTapDetectingImageView *)photoImageView
{
    if (!_photoImageView) {
        _photoImageView = [[BDHTapDetectingImageView alloc] initWithFrame:CGRectZero];
