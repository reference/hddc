...
@interface GPGeologicalSvyLine ()

@end

@implementation GPGeologicalSvyLine
...
- (void)api_loadDetail
{
    ...
    [YXGeologicalSvyLineModel requestDetailWithUUID:self.forumListModel.uuid completion:^(YXGeologicalSvyLineModel * _Nonnull m, NSError * _Nonnull error) {
        if (error) {
...
- (void)setUpUIByModel:(id)model
{
    ...
    //测线名称
    [self.textFields textFieldForTag:4].text = m.svylinename;
    //观测目的
...
- (id)buildBody
{
    YXGeologicalSvyLineModel *body = nil;
    if (self.interfaceStatus == InterfaceStatus_Edit) {
        ...
        body = [YXGeologicalSvyLineModel new];
    }
    //省
    ...
    //测线名称
    body.svylinename = [[self.textFields textFieldForTag:4].text trim];
    //观测目的
    ...
    [self judgeBaseDataWhenFinished:^(BOOL pass) {
        if (pass) {
            ...
            if (self.interfaceStatus == InterfaceStatus_Edit) {
                ...
                t.tableName = NSStringFromClass(YXGeologicalSvyLineModel.class);
                t.encodedData = [[self buildBody] yy_modelToJSONString];
        ...
        if (pass) {
            ...
            [self alertText:@"数据提交成功后将不能修改，您是否确认提交？" sureTitle:@"提交" sureAction:^{
                ...
                void (^submitRequest)(NSString *imgUrls) = ^(NSString *imgUrls) {
                    YXGeologicalSvyLineModel *body = [self buildBody];
