...
@interface GPGeomorphySvyReProf ()

@end

@implementation GPGeomorphySvyReProf
...
- (void)api_loadDetail
{
    ...
    [YXGeomorphySvyReProfModel requestDetailWithUUID:self.forumListModel.uuid completion:^(YXGeomorphySvyReProfModel * _Nonnull m, NSError * _Nonnull error) {
        if (error) {
...
- (void)setUpUIByModel:(id)model
{
    ...
    //所属微地貌面编号
    [self.textFields textFieldForTag:4].text = m.regionid;
    //图切剖面线编号
    [self.textFields textFieldForTag:5].text = m.sliceselfid;
    //测线名称
...
- (id)buildBody
{
    YXGeomorphySvyReProfModel *body = nil;
    if (self.interfaceStatus == InterfaceStatus_Edit) {
        ...
        body = [YXGeomorphySvyReProfModel new];
    }
    //省
    ...
    //所属微地貌面编号
    body.regionid = [[self.textFields textFieldForTag:4].text trim];
    //图切剖面线编号
    body.sliceselfid = [[self.textFields textFieldForTag:5].text trim];
    //测线名称
    ...
    [self judgeBaseDataWithoutIdWhenFinished:^(BOOL pass) {
        if (pass) {
            ...
            if (self.interfaceStatus == InterfaceStatus_Edit) {
                ...
                t.tableName = NSStringFromClass(YXGeomorphySvyReProfModel.class);
                t.encodedData = [[self buildBody] yy_modelToJSONString];
        ...
        if (pass) {
            [self alertText:@"数据提交成功后将不能修改，您是否确认提交？" sureTitle:@"提交" sureAction:^{
                ...
                void (^submitRequest)(NSString *imgUrls) = ^(NSString *imgUrls) {
                    YXGeomorphySvyReProfModel *body = [self buildBody];
