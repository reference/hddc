...
@interface GPGeomorphySvyPoint ()

@end

@implementation GPGeomorphySvyPoint
...
- (void)api_loadDetail
{
    ...
    [YXGeomorphySvyPointModel requestDetailWithUUID:self.forumListModel.uuid completion:^(YXGeomorphySvyPointModel * _Nonnull m, NSError * _Nonnull error) {
        if (error) {
...
- (void)setUpUIByModel:(id)model
{
    ...
    //野外编号
    [self.textFields textFieldForTag:6].text = m.fieldid;
    //高程【米】
...
- (id)buildBody
{
    YXGeomorphySvyPointModel *body = nil;
    if (self.interfaceStatus == InterfaceStatus_Edit) {
        ...
        body = [YXGeomorphySvyPointModel new];
    }
    //省
    ...
    //野外编号
    body.fieldid = [[self.textFields textFieldForTag:6].text trim];
    //高程【米】
    ...
    [self judgeBaseDataWhenFinished:^(BOOL pass) {
        if (pass) {
            ...
            if (self.interfaceStatus == InterfaceStatus_Edit) {
                ...
                t.tableName = NSStringFromClass(YXGeomorphySvyPointModel.class);
                t.encodedData = [[self buildBody] yy_modelToJSONString];
        ...
        if (pass) {
            ...
            [self alertText:@"数据提交成功后将不能修改，您是否确认提交？" sureTitle:@"提交" sureAction:^{
                ...
                void (^submitRequest)(NSString *imgUrls) = ^(NSString *imgUrls) {
                    YXGeomorphySvyPointModel *body = [self buildBody];
